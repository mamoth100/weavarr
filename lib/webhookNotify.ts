...
export async function sendWebhookNotification(title: string, message: string, link?: NotificationLink): Promise<void> {
  if (!WEBHOOK_URL) throw new Error('Webhook notifications are not configured');
  const res = await fetch(WEBHOOK_URL, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ title, message, timestamp: new Date().toISOString(), ...(link ? { url: link.url } : {}) }),
  });
