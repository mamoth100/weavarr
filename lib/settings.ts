...
export const SETTINGS_SCHEMA: SettingField[] = [
  ...
  { key: 'IMPORT_NOTIFY_UNAIRED', label: 'Include Unaired Count in "Ready to Watch" Pings', group: 'App Behavior', secret: false, type: 'boolean', defaultValue: 'true', info: 'When an episode\'s "ready to watch" ping goes out, add how many episodes of that show haven\'t aired yet, like "2 unaired episodes remain". Counts every episode Sonarr has a future air date for, any season. Applies immediately, no restart needed.' },
  ...
  { key: 'ENABLE_EPISODE_WARN', label: 'Big Add Warning', group: 'App Behavior', secret: false, type: 'boolean', defaultValue: 'true', info: 'When a single add or "get more" request would download a pile of episodes, the app asks you to confirm before sending it, so one click can\'t accidentally queue a whole 200-episode series. The threshold below sets how big a pile counts. Applies immediately, no restart needed.' },
  { key: 'EPISODE_WARN_COUNT', label: 'Big Add Warning Threshold (episodes)', group: 'App Behavior', secret: false, info: 'The warning above kicks in when a request would download this many episodes or more. Default 30, must be at least 1. To turn the warning off entirely, use the toggle above.' },
