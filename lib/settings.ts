...
export const SETTINGS_SCHEMA: SettingField[] = [
  ...
  { key: 'ENABLE_AUTO_CLEANUP', label: 'Auto-Delete Watched Episodes', group: 'App Behavior', secret: false, type: 'boolean', defaultValue: 'false', info: 'Deletes an episode\'s file on its own after you have actually watched it (played it, not just marked it watched). The number sets how many days to wait after the watch before deleting, so an accidental play or a change of heart doesn\'t lose the file instantly. Default 3, and 0 deletes on the next hourly check. Never touches Cleanup Excluded Shows, and anything you cleared from Recently Watched is left alone. Applies immediately, no restart needed.' },
