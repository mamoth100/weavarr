...
export const SETTINGS_SCHEMA: SettingField[] = [
  ...
  { key: 'ENABLE_AUTO_CLEANUP', label: 'Auto-Delete Watched Episodes', group: 'App Behavior', secret: false, type: 'boolean', defaultValue: 'false', info: 'Deletes an episode\'s file on its own after you have actually watched it. A finished watch counts, and so does playing past the Cleanup Watched Threshold above. The number sets how many days to wait before deleting, so an accidental play or a change of heart doesn\'t lose the file instantly. Default 3, and 0 deletes on the next hourly check. Never touches Cleanup Excluded Shows, and anything you cleared from Recently Watched is left alone. Applies immediately, no restart needed.' },
  { key: 'AUTO_CLEANUP_DAYS', label: 'Auto-Delete Grace Period (days)', group: 'App Behavior', secret: false },
  { key: 'ENABLE_AUTO_CLEANUP_MARKED', label: 'Auto-Delete Manual Marks Too', group: 'App Behavior', secret: false, type: 'boolean', defaultValue: 'false', info: 'Normally auto-delete only touches episodes with real playback behind them. Turn this on and episodes marked watched by hand in Plex or Jellyfin (no playback recorded at all) also qualify, with the same grace period counted from the mark. Applies immediately, no restart needed.' },
  { key: 'AUTO_CLEANUP_NOTIFY', label: 'Notify on Auto-Deletes', group: 'App Behavior', secret: false, type: 'boolean', defaultValue: 'true', info: 'Sends a ping through your notification channels whenever auto-delete removes episodes, listing exactly what was deleted. Applies immediately, no restart needed.' },
